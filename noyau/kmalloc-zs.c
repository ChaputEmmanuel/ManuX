 ...
 */
enteteBlocMemoire * blocsLibres[MANUX_KMALLOC_ORDRE_MAX] = {NULL};

#ifdef MANUX_KMALLOC_STAT
uint32_t nbPagesAllouees = 0;
uint32_t nbAlloc[MANUX_KMALLOC_ORDRE_MAX] = {0};
uint32_t nbFree[MANUX_KMALLOC_ORDRE_MAX] = {0};
...
void decouperUnBloc(uint16_t o)
{
   ...
   //   printk(PRINTK_DEBUGAGE"On obtient  ordre %d d'adresse 0x%x = %d\n", o, part1, part1);
   //printk(PRINTK_DEBUGAGE"      et un ordre %d d'adresse 0x%x = %d\n", o, part2, part2);
   //   printk(PRINTK_DEBUGAGE"On casse un ordre %d d'adresse 0x%x en \n   0x%x\n   0x%x\n", o, origine, part1, part2);
...
void * kmalloc(size_t n)
{
   ...
   if (enteteBlocAlloue == NULL) {
      ...
      nbAlloc[ordre]++;
#endif
      //      printk(PRINTK_DEBUGAGE "Fin de kmalloc :\n");
      //      kmallocAfficherStatistiques();
