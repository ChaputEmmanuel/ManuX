...
\subsubsection{Remarque préliminaire}

   Avant toute chose, je précise que cette section n'est pas un cours,
elle ne se veut pas complète, elle est juste là pour vous rafraîchir
les idées. Pour le dire plus simplement : allez donc voir votre cours de
   systèmes d'exploitation !

\subsubsection{Remarque liminaire}

   Je vais supposer pour le moment que nous utilisons un système
informatique doté d'un processseur unique, de mémoire, de dispositifs
d'entrée-sortie. A tout instant le microprocesseur peut exécuter une
unique instruction et recevoir une interruption.

   Il me semble en effet important de bien connaître et comprendre
cette configuration simple avant de passer à des choses plus lourdes !


   Je vais donc commencer par amener le problème simplement, en me
fondant sur une technique élémentaire permettant d'introduire du
``pseudo parallélisme'' grâce à du temps partagé fondé sur quelques
outils basiques.
...
\subsubsection{Le traitement d'une interruption}
...
\includegraphics{images/une-tache-seule.png}
...
\includegraphics{images/une-tache-int.png}
...
\subsubsection{Introduction du temps partagé}
...
\includegraphics{images/deux-taches.png}
...
à la gestion de la mémoire, \ldots
...
rêgles simples'' et de s'y tenir. L'ensemble des outils de
...
mettre en place des outils coercitifs obligeant les tâches à
...
\includegraphics{images/memoire-disjointe}
...
\includegraphics{images/memoire-partagee}
...
\includegraphics{images/memoire-disj-et-part}
...
\includegraphics{images/processus-et-threads}
...
\section{L'implantation dans ManuX}
   ...
   ManuX implante et utilise un modèle de mémoire allouée en mode
utlisateur et partagée dans le noyau. En l'absence de gestion de la
mémoire fondée sur la MMU, l'usage exclusif de la mémoire allouée en
...
\end{itemize}
   ...
   Avant de rentrer dans le détail de ces outils, revenons brièvement
sur les spécificités des différentes situations.
   
%-------------------------------------------------------------------------------
%      Les situations à gérer
%-------------------------------------------------------------------------------
\subsection{Les situations à gérer}

   Rappelons-nous que pour le moment ManuX fonctionne sur un système
mono-processeur. Cela signifie que les seules situations
problématiques (c'est-à-dire nécessitant la mise en place de
mécanismes de synchronisation) sont les suivantes

\begin{itemize}
   \item le code de la tâche en cours et le code de traitement de
     l'interruption manipulent des ressources communes ;
   \item la tâche en cours au moment de l'interruption et la
     nouvelle tâche vers laquelle l'ordonnanceur décide de basculer
...
%
%...............................................................................
\subsubsection{Synchronisation entre une tâche et une interruption}

   Un premier élément important vient des différences fondamentales entre
une tâche et le traitement d'une interruption.

   L'exécution d'une tâche peut durer longtemps, elle peut être mise
en pause puis réveillée plus tard, elle est pour cela caractérisée par
un contexte d'exécution (état des registres, de la pile, \ldots) qui
doit être sauvegardé lorsqu'elle est mise en pause et restauré
lorsqu'elle est réveillée. L'exécution d'une tâche ne bloque rien :
elle n'empèche pas le traitement d'une interruption, ni le basculement
vers une autre tâche.

   D'un autre côté, le code de traitement d'une interruption est là
pour gérer une situation particulière, signalée par l'interruption
(par exemple l'arrivée d'une trame sur une interface réseau). Il n'est
lié à aucun contexte particulier qui pourrait être sauvegardé et
repris. Ce code doit s'exécuter rapidement, il doit souvent dialoguer
avec le matériel pour lui préciser que l'événement est pris en
charge. L'exécution de ce code est généralement bloquant pour le
système : il est impossible de basculer vers une tâche, et souvent
impossible même de traiter une autre interruption. Il est évidemment
impossible de mettre en pause l'exécution de ce code.

   Les outils de synchronisation utilisables dans une situation
impliquant une interruption doivent donc être conçus en ce sens.

%
%
%
\paragraph{Masquage des interruptions}

   Une première solution est la possibilité d'interdire les
interruptions (toutes ou certaines) lorsqu'une tâche manipule une
ressource que le traitement d'une interruption pourrait manipuler de
façon incohérente.

%
%
%
\paragraph{Les verrous tournants}

   Les ``verrous tournants'' ({\em spinlocks} en anglais) sont un
outil simple permettant d'assurer une exclusion mutuelle par le biais
d'une éventuelle attente active.
   
%...............................................................................
%
%...............................................................................
\subsubsection{Synchronisation entre deux tâches}

%...............................................................................
%
%...............................................................................
\subsubsection{Réentrance du noyau}
...
implantées dans les fichiers \fichier{include/manux}{atomique}{h} et
\fichier{noyau}{atomique}{c}.
...
%
%
\subsubsection{Affichage des conditions}

   Si la \macromanux{MANUX_ATOMIQUE_AUDIT} est définie
