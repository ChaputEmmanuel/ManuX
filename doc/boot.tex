...
informations fournies par le bootloader au travers des deux variables
\lstinline!signatureBootloader! et \lstinline!_infoSysteme!. Une
difficulté provient du fait que la majorité de ces informations sont
dans une zone mémoire qui risque d'être écrasée lors de
...
intéressent ManuX : la taille mémoire et la ligne de commande.

   La fonction \lstinline!bootloaderInitialiser()! a pour rôle
d'initialiser des variables de ManuX en fonction du contenu de cette
structure (qui est appelée à disparaître).

   Elle doit être invoquée très tôt lors du démarrage de ManuX, a
minima avant l'initialisation de la mémoire.
