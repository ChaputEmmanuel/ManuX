...
ou directement dans le {\tt Makefile}) et le programme {\tt
  outils/makeconfig} est là pour générer un fichier {\tt make.conf}
qui contient ces variables.
...
mémoire de la machine, prêt à être exécuter. Nous allons ici nous
fonder sur une situation ``à l'ancienne'' avec un {\sc pc} doté d'un
...
disquette du {\sc pc}, nous allons donc construire une image de disquette
...
\end{figure} 
   ...
   Ce que j'appelle ici le boot du système se passera donc en deux temps. D'abord le
...
\subsubsection{Démarrage du noyau ({\tt noyau/main.c})}
   ...
   La dernière action du programme d'initalisation est donc l'xécution
